...
def user_sleep_time_correlations(user_id):
    ...
    if len(df_qty_cat) > 0:
        ...
        if isinstance(obs_count, int):
            ...
            logger_apple.info(f"- No weather -")
    try:
        for indVarObj in list_of_arryIndepVarObjects_dict:
            if indVarObj.get('correlationValue') != 'insufficient data':
                logger_apple.info(f"- user_sleep_time_correlations (json dict sent to iOS -")
                logger_apple.info(f"{list_of_arryIndepVarObjects_dict}")
